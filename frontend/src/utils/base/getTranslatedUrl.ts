...
export const getTranslatedUrl = (locale: string, app: IApp): string | null => {
    ...
    let url = `${process.env.BASE_PATH}${localePart}${window.location.search}`;
    if (trans?.url) {
        if (trans?.url?.includes(':slug')) {
            const objTrans = getItemTranslation((app?.item as Record<string, any>)?.attributes || {}, locale);
            if (objTrans?.slug) {
                url = `${process.env.BASE_PATH}${localePart}/${trans.url?.replace(':slug', objTrans?.slug)}${
                    window.location.search
                }`;
            }
        } else {
            url = `${process.env.BASE_PATH}${localePart}/${trans.url
                ?.replace('homepage-en', '')
