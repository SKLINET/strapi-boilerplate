 ...
 * @returns {Object}
 */
export const getDeepPopulate = (uid, { countMany = false, countOne = false, maxLevel = Infinity } = {}, level = 1) => {
    if (level > maxLevel) {
        return {};
    }
    ...
    return Object.keys(model.attributes).reduce(
        (populateAcc, attributeName) =>
            _.merge(populateAcc, getPopulateFor(attributeName, model, { countMany, countOne, maxLevel }, level)),
        {},
    );
};
